...
from byro_directdebit.models import DirectDebitConfiguration, DirectDebit, DirectDebitPayment, DirectDebitState
...
class PrepareDDForm(forms.Form):
    debit_date = forms.DateField(label=_("Debit date"), required=True,
                                 help_text=_("Date on which the debit should become effective. "
    ...
    debit_text = forms.CharField(required=True, label=_('Memo'), help_text=_('Memo/subject that the member will see '
    ...
    sepa_format = forms.CharField(required=True, label=_('SEPA PAIN format'), initial='pain.001.001.03')
    ...
    cor1 = forms.BooleanField(label=_('Issue express debit (COR1)'),
                              required=False,
                              help_text=_("An express debit has a reduced lead "
...
class PrepareDDView(FinTSInterfaceMixin, FormView):
    ...
    def setup(self, *args, **kwargs):
            ...
            for login_pk, connection in connections.items():
                for account in connection.get('accounts', []):
                    if 'iban' in account and account['iban']:
                        ...
                        if not fallback_account: # Randomly select the first one as fallback
                            fallback_account = iban
                            fallback_login_pk = login_pk

                        if account['supported_operations'][FinTSOperations.SEPA_DEBIT_MULTIPLE] \
                            or account['supported_operations'][FinTSOperations.SEPA_DEBIT_MULTIPLE_COR1]:
                            ...
                            if not self.selected_account:
                                ...
                                if connection.get('bank', {}).get('supported_formats', {}):
                                    sup_mul = connection['bank']['supported_formats'].get(FinTSOperations.SEPA_DEBIT_MULTIPLE, None)
                                    sup_mul_cor1 = connection['bank']['supported_formats'].get(FinTSOperations.SEPA_DEBIT_MULTIPLE_COR1, None)
        ...
        if self.selected_sepa_pain_formats:
            self.selected_sepa_pain_formats = set(
                (e[:-4] if e.lower().endswith('.xml') else e).rsplit(':', 1)[-1] for e in self.selected_sepa_pain_formats
            )
    ...
    def get_initial(self):
        ...
        retval['debit_text'] = _('Membership fees for %(organization_name)s' % {
        ...
        if self.selected_sepa_pain_formats:
            l = list(e.lower() for e in self.selected_sepa_pain_formats if e.lower() in SUPPORTED_PAIN_FORMATS)
            if not l:
                messages.warning(self.request, _("No common supported SEPA PAIN format"))
    ...
    def _get_members():
        ...
        return [
            m for m in all_members
            if m.balance < 0 and m.profile_sepa.sepa_direct_debit_state == SepaDirectDebitState.OK
        ]
    ...
    def form_valid(self, form):
        ...
        with atomic():
            debit = DirectDebit(
                ...
                pain_descriptor='urn:iso:std:iso:20022:tech:xsd:' + form.cleaned_data['sepa_format'],
                additional_data = {
            ...
            for member in members:
                ...
                context = {
                    ...
                    'amount': "%.2f %s" % (debit_payment.amount, global_config.currency),
                }
        ...
        return HttpResponseRedirect(
            reverse('plugins:byro_directdebit:finance.directdebit.transmit_dd', kwargs={'pk': debit.pk}))


class TransmitDDView(FinTSInterfaceMixin, SingleObjectMixin, TemplateResponseMixin, View):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.fints_interface:
            debit_meta = self.fints_interface.sepa_debit_init(self.object.additional_data['login_pk'])
        ...
        context['debit_sum'] = self.object.payments.aggregate(amount=Sum('amount'))['amount']
...
class TransmitDDTANView(FinTSInterfaceMixin, SingleObjectMixin, TemplateResponseMixin, View):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        if self.fints_interface:
            tan_props = self.fints_interface.tan_request_init(self.object.additional_data['login_pk'], self.kwargs['transfer_uuid'])
